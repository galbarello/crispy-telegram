...
| `comparison` `cycle` `chips` `pyramid` `funnel` `quadrant` `pillars` `hub` `timeline` `swimlane` `tree` `venn` `spectrum` `decision` `rings` | metaphor blocks — one build recipe each in `references/block-catalog.md` (real connectors, area-nested cells, never `create_table`). |
